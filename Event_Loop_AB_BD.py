...
class Loop(object):
    __slots__ = ['epoll', 'free_sockets', 'fileno_socket', 'active_sockets', 'epoll_delay', 'gen', 'gen_list', 'gen_loop', 'next_gen_loop', 'next_gen_periodic', 'next_gen_socket_closed', 'timer', 'timer_thread', 'max_active_sockets', 'timeout', 'loop_host_dict', 'timer_loop_others', 'loop_delay']

    def __init__(self, connect_timeout=20, epoll_delay=0.0001, loop_host_dict={}, loop_delay=0.1015):
        ...
        self.gen_list = set()
        self.gen_loop = None
        self.next_gen_loop = deque()
        self.next_gen_periodic = set()
        self.next_gen_socket_closed = set()
        self.max_active_sockets = 21
        self.timeout = 10
        self.loop_host_dict = loop_host_dict
        self.timer_loop_others = set()
        self.loop_delay = loop_delay

        for host, value in self.loop_host_dict.items():
            for i in range(value['create_free_sockets']):
    ...
    def add_future_loop(self, gen, delay=None):
        if self.gen_loop:
            self.timer_loop_others.add((_Timer_loop_other, gen, delay))
        else:
            self.gen_loop = gen

    def request(self, host=None, url=None, headers=[], data=None, access_token='', timeout=20, after_idle_sec=1, interval_sec=3, max_fails=5, is_http=False):
        if self.free_sockets[host]:
            ssock = self.free_sockets[host].pop(0)
            ssock.gen = self.gen.pop()
            if self._send_data(ssock, url, host, headers, data, access_token):
                self.epoll.register(ssock.fileno(), select.EPOLLIN |select.EPOLLERR | select.EPOLLET)
        else:
            ssock = self.create_socket(host, is_http=is_http)
            if ssock:
                ssock.gen = self.gen.pop()
                ssock.dict_post = {'url':url, 'headers':headers, 'data':data, 'access_token':access_token}
            else:
                self.next_gen_socket_closed.add(self.gen.pop())

    def create_socket(self, host, after_idle_sec=1, interval_sec=3, max_fails=5, is_http=False):
        if is_http:
            port = 80
            ssock = MySocket(socket.AF_INET,socket.SOCK_STREAM)
            ssock.setsockopt(socket.IPPROTO_TCP, socket.TCP_NODELAY, 1)  # 禁用 Nagle 算法，减少延迟
            ssock.setsockopt(socket.SOL_SOCKET, socket.SO_KEEPALIVE, 1) # 在客户端开启心跳维护
            ssock.setsockopt(socket.IPPROTO_TCP, socket.TCP_KEEPIDLE, after_idle_sec)  # 设置连接上如果没有数据发送的话，多久后发送keepalive探测分组，单位是秒
            ssock.setsockopt(socket.IPPROTO_TCP, socket.TCP_KEEPINTVL, interval_sec)  # 前后两次探测之间的时间间隔，单位是秒
            ssock.setsockopt(socket.IPPROTO_TCP, socket.TCP_KEEPCNT, max_fails)  # 关闭一个非活跃连接之前的最大重试次数
        else:
            port = 443
    ...
    def _send_data(self, ssock, url, host, headers, data, access_token):
        fileno = ssock.fileno()
        headers = ''.join(headers)
        data = f"POST {url} HTTP/1.1\r\nHost:{host}\r\nContent-Length:{len(data)}\r\n{access_token}{headers+ssock.cookie if 'Cookie' not in headers and ssock.cookie else headers}\r\n{data}".encode()
        try:
        ...
        for fileno, event in events:
            ...
            if event & select.EPOLLIN:
                ...
                if getattr(sock, 'dict_post', None):
                    if self._send_data(sock, sock.dict_post['url'], sock.host, sock.dict_post['headers'], sock.dict_post['data'], sock.dict_post['access_token']):
                        del sock.dict_post
        ...
        while 1:
            for i in range(100):
                ...
                while self.next_gen_loop:
                    next(self.next_gen_loop.popleft())
                for host, value in self.loop_host_dict.items():
            ...
            if self.next_gen_socket_closed:
                self.send_gen(self.next_gen_socket_closed.pop(), value='')
    ...
    def _run_timer(self):
        ...
        for timer_loop_other, gen, delay in self.timer_loop_others:
            t = threading.Thread(target=timer_loop_other(self).run,args=(gen, delay))
            t.setDaemon(True)
            t.start()
...
class _Timer(object):
    ...
    def run(self):
        if self.loop.gen_loop:
            while 1:
                for i in range(10):
                    self.loop.next_gen_loop.append(self.loop.gen_loop)
                    time.sleep(self.loop.loop_delay)
...
class _Timer_loop_other(object):
    __slots__ = ['loop']

    def __init__(self, loop):
        self.loop = loop

    def run(self, gen, delay):
        while 1:
            self.loop.next_gen_loop.append(gen)
            time.sleep(delay)
