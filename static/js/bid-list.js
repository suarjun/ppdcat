function gotoPage(a, page, policy_name, start_date, end_date, total_page)
    {$.post("/account/bid-list/{{ current_user.Name }}",
        {offset:20*(page-1),
...
function replaceList(record_list)
    ...
    record_list.forEach(function(record){
        tbody.append('<tr id="list"><td style="text-align:center;">'+record["PolicyName"]+'</td><td style="text-align:center;"><a class=url href="https://invest.ppdai.com/loan/info/'+record["ListingId"]+'" target=_blank>'+record["ListingId"]+'</a></td><td style="text-align:center;">'+record["ParticipationAmount"]+'</td><td style="text-align:center;">'+record["BidTime"]+'</td><td style="text-align:center;">'+record["User"]+'</td></tr>')
        });
    };
...
function replaceGagination(policy_name, start_date, end_date, current_page, total_page)
    ...
    for (var i = 1; i <= total_page; i ++)
        {if (1 <= i && i <= 2 || -1 <= i-total_page && i-total_page <= 0 || -1 <= i-current_page && i-current_page <= 4)
            {prev = i;
            if (i == current_page)
